...
static void map_framebuffer_address (uint64_t phys_addr) {
    ...
    // virtual mapping Since we already set up the high virtual address mapping
    // in boot.asm, we can access the framebuffer through the high virtual
    // address
    ...
    while (current_tag_ptr < end_of_tags) {
        ...
        switch (tag->type) {
            case MULTIBOOT_TAG_TYPE_END:
                duts("Found end tag. Multiboot parsing complete.");
                return;
...
void kernel_main (void* mb_info) {
    // In this case, we must use use_debug instead of the funcs that check debug
    if (use_debug) {
